...
class subscription {
  /**
   * @param { object } reg - the register object from babble-drachtio-registrar
  ...
  */
  ...
  async _onnotify( req, res ) {
    ...
    const authorization = this._auth.parseauthheaders( req )
    if( !this._user ) {
    ...
    if( !this._auth.equal( req ) ) {
      const regauth = this.reg.getauth( req )
      if( regauth ) {
        this._auth = regauth
      }
    }
