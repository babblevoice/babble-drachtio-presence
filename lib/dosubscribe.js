...
class RegSubscription {
  constructor( reg, dialog ) {
    ...
    this.timer = setTimeout( () => this.timeout(), reg.expiresin * 1000 )
  }
  ...
  timout() {
    if( subscribeperregister.has( this.reg.uuid ) ) {
      let sub = subscribeperregister.get( this.reg.uuid )
      deleteregsub( this.reg )
      sub.destroy()
    }
  }
}
