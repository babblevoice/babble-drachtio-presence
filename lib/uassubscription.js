...
class subscription {
  ...
  constructor( req, res, options ) {
    ...
    */
    ...
    if( !this._options.authtimeout ) this._options.authtimeout = authtimeout
    ...
    */
    ...
    if( isNaN( this.expires ) ) {
      this.destroy()
      res.send( 400, "No valid expires" )
      return
    }

    if( !req.has( "call-id" ) ) {
      this.destroy()
      res.send( 400, "No valid call-id" )
      return
    }
  ...
  async #updateauth( req, res ) {
    ...
    if( !this._user || !this._auth.verifyauth( req, this._authorization, this._user.secret ) ) {
      if( this._auth.stale ) {
        clearTimeout( this._timers.subauth )
        this._timers.subauth = setTimeout( this._ontimeout.bind( this ), this._options.authtimeout )
        this._auth.requestauth( req, res )
        return false
      }
   ...
   * @returns { boolean }
   */
  #checkexpires( req, res ) {

    const expires = subscription.getexpires( req )

    if( isNaN( expires ) ) {
      res.send( 400, "Bad Expires" )
      this.destroy()
      return false
    }
    ...
    clearTimeout( this._timers.subexpire )
    this._timers.subexpire = setTimeout( () => {
      this.destroy()
    }, expires * 1000 )
  ...
  async _update( req, res ) {
    ...
    if( !( await this.#updateauth( req, res ) ) ) {
      return false
    }
    this._authed = true

    if( false === this.#checkexpires( req, res ) ) return false

    /* It is not mandatory to require this header - default when one not provided */
    let accept = "application/pidf+xml"
    if( req.has( "Accept" ) ) accept = req.get( "Accept" )
  ...
  async notifyvoicemail( info ) {
    ...
    const opts = {
      ...
      "headers": {
        "Content-Type": this.accept,
        "Subscription-State": "active;expires=" + this.expires,
