...
class subscription {
  ...
  */
  ...
  async _update( req, res ) {
    ...
    const inaccepts = req.get( "Accept" ).split( "," )
  ...
  notifyregistration( info ) {
    ...
    if( "application/pidf+xml" === this.accept ) this.#handlepidfxml( info )
    else if( "application/xpidf+xml" === this.accept ) this.#handlexpidfxml( info )
  }
}
