...
/*
Store dialogs per registration.
*/
...
module.exports.reg = ( options ) => {
  ...
  return async ( reg ) => {
    ...
    let dialog
    try {
      dialog = await options.srf.createUAC( reg.contacts[ 0 ], {
        "method": "SUBSCRIBE",
        "headers": {
          "To": `<sip:${reg.auth.username}@${reg.auth.realm}>`,
          "From": `<sip:${reg.auth.username}@${reg.auth.realm}>`,
          "Event": "presence",
          "Expires": reg.expiresin,
          "Accept": "application/pidf+xml"
        }
    ...
    let auth = new sipauth.auth( options.proxy )
    let user
    dialog.on( "notify", async ( req, res ) => {
      ...
      */
      ...
      if( false === s ) {
        ...
        options.em.emit( "presence.status.in", {
          ...s,
          ...{
            "entity": req.auth.username + "@" + req.auth.realm,
            "source": {
              "event": "NOTIFY",
