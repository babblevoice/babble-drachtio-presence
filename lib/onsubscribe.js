...
class Subscription {
  ...
  cleanup() {
    ...
    removesubscription( this.entity, this.dialog )
  }
...
class SubscriptionCollection {
  ...
  constructor( entity, dialog, expires ) {
    ...
    this.subscriptions.set( dialog.id, new Subscription( entity, dialog, expires ) )
  }
  ...
  remove( dialog ) {
    if( this.subscriptions.has( dialog.id ) ) {
      ...
      sub.cleanup()
      this.subscriptions.delete( dialog.id )
    }
  }
  ...
  voicemail() {
    for( var [ id,  d ] of this.subscriptions ) {
      notifyme( this.entity, "application/simple-message-summary", d.dialog )
    }
  }
}
...
*/
function notifyme( entity, contenttype, dialog, expires ) {
  /*
...
/* Signal a change in voicemail */
module.exports.voicemail = ( entity ) => {
  if( subscriptions.has( entity ) ) {
    subscriptions.get( entity ).voicemail()
  }
}
      ...
      options.srf.createUAS( req, res, {
        ...
        .then( ( dialog ) => {
          ...
          acceptedformats.forEach( ( ct ) => {
            notifyme( entity, ct, dialog, expires )
          } )
        } )
    } )
  }
