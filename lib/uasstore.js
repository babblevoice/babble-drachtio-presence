...
module.exports.set = ( subscription ) => {
  ...
  const user = subscription.uri.user + "@" + subscription.uri.host
...
@return { Map | object | false } a map of subscriptions for this user
*/
...
/**
Return a list of subscriptions by user
@return { Array } Array of fq users
*/
module.exports.getsubs = () => {
  return Array.from( bysubscribed.keys() )
}
...
module.exports.delete = ( subscription ) => {
  ...
  const ourmap = bysubscribed.get( user )
